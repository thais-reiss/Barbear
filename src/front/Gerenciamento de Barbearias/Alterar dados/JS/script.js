/*const idBarbearia = prompt("Digite o ID da barbearia que deseja atualizar:");*/
let step = 1;
...
window.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    if (!response.ok) {
      alert("Erro ao carregar os dados da barbearia a ser atualizada.");
      return;
    }
    const barbearia = await response.json();

    document.getElementById("EmailBarbearia").value = barbearia.emailBarbearia || "";
    document.getElementById("SenhaBarbearia").value = barbearia.senhaBarbearia || "";
    document.getElementById("NomeBarbearia").value = barbearia.nomeBarbearia || "";
    document.getElementById("CNPJ").value = barbearia.cnpj || "";
    document.getElementById("Telefone").value = barbearia.telefoneBarbearia || "";

    const endereco = barbearia.enderecoBarbearia;
    const regex = /CEP:\s*(\d+),\s*Rua:\s*([^,]+),\s*Bairro:\s*(.+)/;

    const match = endereco.match(regex);

    const cep = match[1];
    const rua = match[2];
    const bairro = match[3];
    ...
    preencherCheckboxes(idBarbearia);
  } catch (error) {
    console.error("Erro ao carregar dados da barbearia a ser atualizada: ", error);
    /*alert("Erro ao carregar os dados da barbearia.");*/
  }
});

function preencherCheckboxes(idBarbearia) {
  fetch(`http://localhost:8080/barbearia/${idBarbearia}`)
    .then(response => {
      if (!response.ok) {
        console.log('Erro ao buscar horários da barbearia');
      }
      return response.json();
    })
    .then(data => {
      const horariosDisponiveis = data.horario;
      console.log("Horários: ", horariosDisponiveis);
      document.querySelectorAll('input[name="horario"]').forEach(checkbox => {
        if (horariosDisponiveis.includes(checkbox.value)) {
    ...
    if (step === 1) {
      ...
      try {
        const response = await fetch(
          `http://localhost:8080/barbearia/${idBarbearia}`,
          {
            method: "PUT",
            headers: {
        ...
        if (!response.ok) {
          ...
          console.error("Erro ao alterar a barbearia. ", mensagemDeErro);
          alert("Erro ao alterar a barbearia.");
          return;
        }
